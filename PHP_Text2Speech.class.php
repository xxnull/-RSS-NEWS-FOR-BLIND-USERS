	...
	function speak($text, $lang = NULL) {
		...
		if (strlen($text) > $this->maxStrLen) {
			...
			if (!file_exists($file)) {
				...
				// Download new file
				$this->download("http://api.voicerss.org/?key=8b0448397d024c16b58a8f8ade140c52&src={$text}&hl=es-es&total={$this->wordCount}&idx=0&textlen={$this->textLen}", $file);
				
				//http://api.voicerss.org/?key=8b0448397d024c16b58a8f8ade140c52&src={$text}&hl=es-es&total={$this->wordCount}&idx=0&textlen={$this->textLen}
				
				//$this->download("http://translate.google.com/translate_tts?tl=en&q={$text}&tl={$this->lang}&total={$this->wordCount}&idx=0&textlen={$this->textLen}", $file); 
			}
		}
